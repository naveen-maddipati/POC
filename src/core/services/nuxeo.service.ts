...
export class NuxeoService {
   ...
   */
  getChildren(pathOrId: string, pageSize: number = 20, currentPage: number = 0): Observable<IApiResponse<INuxeoDocuments>> {
    return this.ensureConnection().pipe(
      switchMap(() => {
        const startTime = performance.now();
        const params = {
   ...
   * Perform NXQL query
   */
  query(nxql: string, pageSize: number = 20, currentPage: number = 0): Observable<IApiResponse<INuxeoDocuments>> {
    return this.ensureConnection().pipe(
      switchMap(() => {
        const startTime = performance.now();
        const params = {
